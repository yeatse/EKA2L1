...
The final emulator source matches commit `efb24fc9a`. Its Release simulator build passed the default regression with 12 passes and zero failures; screenshots and guest logs were reviewed. `ekatests` completed with 5,907 assertions in 232 test cases. The independent private service passed all 21 tests. The original device identity, account cookie, saved games and logging settings were restored after the two-account validation; the requested hosts mapping remains enabled.
