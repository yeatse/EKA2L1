...
namespace eka2l1::drivers {
    ...
    void graphics_command_builder::copy_framebuffer_to_texture(drivers::handle texture, const std::uint8_t level,
        ...
        const eka2l1::vec2 &size, const bool copy_source_alpha) {
        command *cmd = list_.retrieve_next();

        cmd->opcode_ = graphics_driver_copy_framebuffer_to_texture;
        cmd->data_[0] = texture;
        cmd->data_[1] = static_cast<std::uint64_t>(level)
            | (static_cast<std::uint64_t>(static_cast<std::uint8_t>(face_index)) << 8)
            | (static_cast<std::uint64_t>(copy_source_alpha) << 16);
