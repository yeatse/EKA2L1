...
5. **iOS 前端从不应用日志过滤器 + 非 `BUILD_FOR_USER` 默认 `*:trace` → 同步刷盘洪水把 CPU 打满**（N97/S60v5 点 Calculator 整屏黑 + CPU 100% 的**主因**）。定位链路：① `IosEmulator::startWithDocumentsPath` 只调了 `log::setup_log(nullptr)`，从没像 Qt（`qt/src/state.cpp:78`）那样调 `log::filterings->parse_filter_string(conf.log_filter)`；而 `log_filterings` 默认构造就是 `reset_all(spdlog::level::trace)`——所有 log class 全在 trace 通过。② iOS app 不是 `CI` → `BUILD_FOR_USER` 构建，`config::config.cpp` 里把 `*:trace` 降级为 normal-use preset 的那段是 `#if BUILD_FOR_USER`，于是 `conf.log_filter` 本身也停在 `LOG_FILTER_DEBUG_PRESET = "*:trace"`。③ 结果每次 guest 上下文切换（`scheduler.cpp:128` 残留的 stage-3.2.1 诊断 `LOG_INFO(KERNEL, "Switch to thread …")`）和每条 dyncom VFP 子操作（`vfp/*.cpp`，class `[CPU.DynCom]`）都被记录，且 spdlog `flush_on(spdlog::level::debug)` 让每条 ≥debug 的消息都**同步 flush 到文件**。轻量 S60v3（N95）勉强挺过去，但 S60v5（N97：`ecomserver`/`AknIconSrv`/`cdlserver` 启动风暴 + 大量浮点 AVKON 排版）被刷盘洪水拖死——点 Calculator 后约 **6s 打出 ~43 万行日志**，CPU 全耗在格式化 + 同步 I/O，guest 几乎不前进、出不了帧，表现为"黑屏 + CPU 100%"。修法（仅动 iOS 前端 `IosEmulator.mm`）：`conf.deserialize()` 后镜像 `BUILD_FOR_USER` 的降级——`log_filter` 为空或等于 debug preset 时，按 `extensive_logging` 选 `LOG_FILTER_DEBUG_PRESET` 或 `LOG_FILTER_NORMAL_USE_PRESET`，并**额外追加 `CPU:warn CPU.DynCom:warn CPU.12L1R:warn`**（iOS 跑 dyncom，normal preset 仍把 CPU class 留在 trace，浮点重的 guest 会刷爆 VFP trace），再 `log::filterings->parse_filter_string(...)` 真正应用；`applyConfigSnapshot` 改 `logFilter` 时也即时重应用（无需重启）。验证：N97 点 Calculator 后日志从 ~43 万行/6s 降到 ~220 行，`Switch to thread`/VFP trace 归零。**注意：此修复只消除"日志洪水导致的 100% CPU"，N97 Calculator 自身仍黑屏——剩余阻塞见下方已知风险的 S60v5 FEP/Pti 项。** 顺带记录：`scheduler.cpp:128` 的 `Switch to thread` 是 stage-3.2.1 遗留的临时诊断 `LOG_INFO`，本应清掉（被 `Kernel:Warn` 过滤后无格式化开销，故本次未动共享内核代码，但值得后续移除）。
6. **N95 Snakes 卡在 splash / `E32USER-CBase 46` stray signal**：现象是在 N95 / S60v3 上启动 Snakes (`uid=0x2000730F`) 后 splash 动画阶段 request semaphore 与 active scheduler 状态失衡，最后 `CActiveScheduler::WaitForAnyRequest` 被额外信号唤醒却没有 ready active object，触发 CBase 46 stray-signal panic，随后 graphics command list 被半途打断。修法不是 Snakes UID 特判，而是三处通用 request/notify 语义修正：① `thread::sleep()` 作为 HLE frame pacing sleep 不再阻塞式消费 guest request semaphore，也不在 wakeup 时补发 request signal，改为纯 scheduler sleep/dewait；② `wait_for_any_request()` 只在确认当前 PC 是 direct `User::WaitForAnyRequest` stub 且 active scheduler 无 ready request 时吸收 stale signals，避免误吞 `User::WaitForRequest(TRequestStatus&)` wrapper 或任意同步 wait，同时当 semaphore count 已耗尽但 active scheduler 已有 ready active object 时直接返回给 guest 调度该 AO；③ `screen_post_transferer::complete_notify()` 从 timing thread 完成 vsync notify 前先移出 pending list，并在 kernel lock 下写 `TRequestStatus` + signal，避免跨线程无锁完成请求。说明：这仍沿用 EKA2L1 现有“request semaphore + guest `TRequestStatus`”模型，不是一次性重做 per-request completion 队列；但改动不含 Snakes 名称/UID 分支，目标是让共享 semaphore 模型在 direct active-scheduler wait、sync wait wrapper 与 HLE pacing sleep 三者之间保持通用平衡。验证（2026-06-01，iPhone 16 Pro iOS 26.5 simulator，N95 device index 0）：清理所有 `REQDBG` 临时诊断后 `xcodebuildmcp simulator build` 通过；Snakes 连续 3 次由 `-LaunchAppUID 0x2000730F` 进入主菜单；Calculator (`uid=0x10005902`) 回归正常渲染；日志无 `REQDBG`、`Thread Snakes panicked`、`E32USER-CBase`、`Access violation`、`Corrupted graphics`、`Active scheduler dump`。
...
| 日期 | 改动 |
|------|------|
| 2026-06-01 | 修复 N95 Snakes splash 后卡死 / `E32USER-CBase 46` stray-signal panic。根因是 HLE frame pacing sleep 曾阻塞式消费并补发 guest request signals，叠加 `WaitForAnyRequest` 对 stale semaphore signals 与 ready active object 的区分不足，以及 vsync notify completion 可从 timing thread 无 kernel lock 完成 `TRequestStatus`，让 request semaphore 与 guest active scheduler 状态失衡。修复后不含 Snakes UID 特判：`thread::sleep()` 改为纯 scheduler sleep/dewait；`wait_for_any_request()` 只在 direct `User::WaitForAnyRequest` stub 上吸收 stale signals，并在已有 ready active object 时不再继续阻塞；vsync notify completion 改到 kernel lock 下完成。验证：iOS 26.5 simulator / N95 上 Snakes (`0x2000730F`) 连续 3 次进入主菜单，Calculator (`0x10005902`) 回归正常，日志无临时诊断、panic、CBase、AV、corrupted graphics 或 active scheduler dump。 |
