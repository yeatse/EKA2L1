...
| 2026-07-13 | **修复 Asphalt 6 真机 watchdog 卡死与早期开场纯黑**：GitHub Actions 拉取 TestFlight `260737` 的匹配 dSYM，三份 `.ips` 精确收敛到同一 kernel↔CoreAudio 锁序环；buffer 通知改用 `try_lock` + 后续 callback 重试，消除 `AudioOutputUnitStop` 在 guest 持 kernel lock 时反等 render callback。早期片头另因 iOS 打包仅收 `_general.dll`、漏掉已有 `mediaclientvideo_v100.dll`，改为 bundle/staging 全部版本化 patch DLL。新增 Asphalt 回归断言真实 Gameloft 电影帧并等待两段电影完整结束。Release：Asphalt **9/9**、标准 **8/8**、Angry Birds **5/5**；真机复测待设备在线。详见 [`docs/ios-asphalt6-x7.md`](./docs/ios-asphalt6-x7.md)。 |
| 2026-07-13 | **打通 X7 Asphalt 6 到实际比赛**：补齐 SCDV `EColor16MAP` + `MSurfaceId`/旋转、Belle executive/LDD、ROM 非 XIP E32 与无盘符 rooted DLL 查找、MMF A3F/ALF、screen mode、Accessory modern IPC、Loader PDD 等通用兼容缺口；无游戏 UID 特判。新增可复现的 ABI-preserving SCDV 制作脚本与 `ios_regression_test.sh asphalt6` 回归。Release 实测进入 Nassau Normal Race；标准回归连续两轮 **8/8**，Angry Birds **5/5**。详见 [`docs/ios-asphalt6-x7.md`](./docs/ios-asphalt6-x7.md)。 |
| 2026-07-11 | **修复 5320 内置 Camera 真机 `E32USER-CBase 49`**：根因链包含三个通用缺陷。①ROM patch 的 `SVC #0xFF` trampoline 从未同步的 saved thread context 读写 PC，导致 lookup miss 且跳过 ecam patch；改为基于 live CPU core 按 ARM/Thumb 宽度还原 SVC 地址并直接跳转。②补齐缺失的 `ECamDuplicate`（dispatch `0x71`），共享 camera 实例/handle 并返回缓存 info。③iOS camera 声明 `CAPTURE_OPTION_ALL` 却拒绝部分标量参数，guest `Leave` 后在旧 ROM 低地址 patch DLL 展开路径中违规访问；为尚无原生映射的 optical zoom/contrast/brightness/white balance 提供实例级缓存 get/set。清理诊断代码后的最终真机包运行 Camera 约 55 秒，日志无 panic、trampoline miss、dispatch 113 缺失、unsupported parameter 或 access violation；Release 模拟器回归连续两轮 **8/8**。详见 [`docs/ios-5320-camera-panic49.md`](./docs/ios-5320-camera-panic49.md)。 |
